...
* **Main Data**: `data/scatter/scatter.csv`（ラベル, X, Y）
* **Category Data**: `data/category/*.csv`
...
* **左ペイン（可変）**: 散布図描画エリア（Matplotlib Canvas）
* **右ペイン（固定）**: 操作パネルおよび情報表示エリア

#### グラフ描画エリア詳細

* 表示要素は「散布図・回帰線・情報表示」のみとする
* **フレーム（軸）**: 表示する
* **補助線**: 表示しない
* **グリッド**: 表示あり（点線・薄色）
...
* CSVの再読み込み
* 設定値（軸範囲・ラベル・回帰線ON/OFF）の反映
* 散布図の再描画

#### 情報表示

* **グラフ外の情報表示エリア（右ペイン内・スクロール可）** に以下を表示する

  * 回帰式: `y = ax + b`
  * 決定係数: `R²`
* グラフ描画エリア内には数式・数値テキストを表示しない
* 情報表示は散布図更新のたびに再計算・再描画される
